//+------------------------------------------------------------------+
//|  SMC_Complete.mq4                                                |
//|  MQL4 | BOS/CHoCH | OBs | FVG | EQH/L | PD | TLZ | OrderFlow  |
//+------------------------------------------------------------------+
...
input string _S2        = "══ Order Blocks (PAC) ══";
...
input string _S3        = "══ Order Flow OBs ══";
input int    PivotLen   = 5;
input int    MaxOFs     = 5;
input bool   HideOverlap    = true;
input bool   ShowLabelsDef  = true;
input bool   ShowStrongDef  = true;
input int    BufferSize     = 5;

input string _S4        = "══ Fair Value Gaps ══";
input bool   ShowFVG    = true;
input int    MaxFVG     = 5;

input string _S5        = "══ Premium / Discount ══";
input bool   ShowPD     = true;

input string _S6        = "══ Trend Line Zones ══";
input bool   ShowTLZ    = true;
input color  BullTLZCol = C'0,150,100';
input color  BearTLZCol = C'200,50,60';

input string _S7        = "══ Panel ══";
input int    PanelX     = 20;
input int    PanelY     = 30;
...
#define BT_IOBUL  P"bIOBul"
#define BT_IOBER  P"bIOBer"
#define BT_IOLBL  P"bIOLbl"
#define BT_IOSTR  P"bIOStr"
#define BT_IOOVLP P"bIOOvlp"
...
// STRUCTS
//═══════════════════════════════════════════════════════════════════
struct Ev  { datetime tBreak, tLevel; double level; int type; bool isSwing; };
struct OBD { double top, btm; datetime t0; double volPct; bool bull, isSwing, mit; };
struct FVD { double top, btm; datetime t0; bool bull, mit; };
struct EqD { double price; datetime t0; bool isHigh; };
struct IOOB{ double top, btm; datetime t0; double str; bool bull, mit; };

//═══════════════════════════════════════════════════════════════════
// STATE — PAC Market Structure
//═══════════════════════════════════════════════════════════════════
int      iDir=0;
double   iBosHi=EMPTY_VALUE, iBosLo=DBL_MAX;
...
int      sDir=0;
double   sBosHi=EMPTY_VALUE, sBosLo=DBL_MAX;
...
// STATE — IOFlow OBs
IOOB g_ioOB[MAX_IO]; int g_ioN=0;
double   g_ioHi=EMPTY_VALUE, g_ioLo=EMPTY_VALUE;
int      g_ioHiI=-1, g_ioLoI=-1;
double   g_ioPC=EMPTY_VALUE;

// Draw tracking (unified)
string gDr[MAX_OB*3 + MAX_EV*2 + MAX_IO*2 + 80];
int    gDrCnt=0;

// Toggles
bool tMSInt=true, tMSSWg=true, tOBInt=true, tOBSWg=true;
bool tEQHL=true,  tFVG=true,   tPD=true,    tTLZ=true;
bool tIOBul=true, tIOBer=true, tIOLbl=true, tIOStr=true, tIOOvlp=true;
bool gFRec=false;
...
int OnInit() {
   ...
   tIOLbl=ShowLabelsDef; tIOStr=ShowStrongDef; tIOOvlp=HideOverlap;
   ClearAll();
   CreatePanel();
...
void ClearAll() {
   ...
   g_ioN=0; g_ioHi=EMPTY_VALUE; g_ioLo=EMPTY_VALUE;
   g_ioHiI=-1; g_ioLoI=-1; g_ioPC=EMPTY_VALUE;
}
...
int OnCalculate(const int rt, const int pc,
   ...
   int minBars = MathMax(SwgLen*2+5, PivotLen*2+5);
   if (rt < minBars) return 0;

   bool full = (pc==0 || gFRec);
   if (full) { ClearAll(); gFRec=false; DelDraw(); }
   int start = full ? MathMax(SwgLen*2, PivotLen*2)
                    : MathMax(pc-1, MathMax(SwgLen*2, PivotLen*2));
   ...
   for (int i=start; i<rt; i++) {
      ...
      // ── IOFlow: Pivot detection ──────────────────────────────────
      if (iob >= PivotLen) {
         double ph2 = PivH(high, iob, PivotLen, rt);
         double pl2 = PivL(low,  iob, PivotLen, rt);
         if (ph2 > 0) { g_ioHi = ph2; g_ioHiI = iob; }
         if (pl2 > 0) { g_ioLo = pl2; g_ioLoI = iob; }
      }
      ...
      // ── IOFlow: BOS detection + OB registration ──────────────────
      if (g_ioPC != EMPTY_VALUE) {
         bool bBOS = (g_ioHi != EMPTY_VALUE && g_ioPC <= g_ioHi && c > g_ioHi);
         bool sBOS = (g_ioLo != EMPTY_VALUE && g_ioPC >= g_ioLo && c < g_ioLo);

         if (bBOS || sBOS) {
            int ref  = bBOS ? g_ioHiI : g_ioLoI;
            int sLen = i - ref;
            ...
            if (bBOS) {
               ...
               for (int s=1; s<=sLen; s++) {
                  ...
                  if (close[bi]<open[bi] && (minL==EMPTY_VALUE || low[bi]<minL)) {
                     minL=low[bi]; oT=high[bi]; oB=low[bi]; oI=bi; oV=(double)tvol[bi];
                  }
               }
               g_ioHi=EMPTY_VALUE;
            } else {
               ...
               for (int s=1; s<=sLen; s++) {
                  ...
                  if (close[bi]>open[bi] && (maxH==EMPTY_VALUE || high[bi]>maxH)) {
                     maxH=high[bi]; oT=high[bi]; oB=low[bi]; oI=bi; oV=(double)tvol[bi];
                  }
               }
               g_ioLo=EMPTY_VALUE;
            }

            if (oI>=0 && oT!=EMPTY_VALUE && g_ioN<MAX_IO) {
               double vS = IOVolSMA(tvol, oI, 20);
               double rV = (vS>0) ? oV/vS : 1.0;
               double oH = oT - oB;
               double bd = bBOS ? (c-oT) : (oB-c);
               if (bd<0) bd=0;
               g_ioOB[g_ioN].top=oT; g_ioOB[g_ioN].btm=oB;
               g_ioOB[g_ioN].t0=time[oI];
               g_ioOB[g_ioN].str=IOStr(oH, bd, rV);
               g_ioOB[g_ioN].bull=bBOS; g_ioOB[g_ioN].mit=false;
               g_ioN++;
            }
         }
      }
      ...
      // ── IOFlow: Mitigation ───────────────────────────────────────
      for (int k=0; k<g_ioN; k++) {
         if (g_ioOB[k].mit) continue;
         if ( g_ioOB[k].bull && low[i]  < g_ioOB[k].btm) g_ioOB[k].mit=true;
         if (!g_ioOB[k].bull && high[i] > g_ioOB[k].top) g_ioOB[k].mit=true;
      }
      ...
      for (int k=0; k<gOBCnt; k++) {
         ...
         if (OBMitMethod==2) { double av=(gOBs[k].top+gOBs[k].btm)/2.0; mit=gOBs[k].bull?close[i]<av:close[i]>av; }
   ...
   if (gOBCnt>250) CompactPACOBs();
   if (gFVCnt>120) CompactFVs();
   if (g_ioN>400)  CompactIOOBs();
   DrawAll(time, high, low, rt);
   return rt;
}

//═══════════════════════════════════════════════════════════════════
// PAC — Pivot state updates
//═══════════════════════════════════════════════════════════════════
void OnIntHigh(double px, datetime t, int bar) {
   if (iDir>=0) { if (iBosHi==EMPTY_VALUE||px>iBosHi) { iBosHi=px; iBosHiT=t; iBosHiBar=bar; } }
   if (iDir<0)  { if (iChHi==EMPTY_VALUE||px>iChHi)   { iChHi=px; iChHiT=t; }
                  if (iChHi!=EMPTY_VALUE&&px<iChHi)    iCfLh=true; }
}
void OnIntLow(double px, datetime t, int bar) {
   if (iDir<=0) { if (iBosLo==DBL_MAX||px<iBosLo) { iBosLo=px; iBosLoT=t; iBosLoBar=bar; } }
   if (iDir>0)  { if (iChLo==DBL_MAX||px<iChLo)   { iChLo=px; iChLoT=t; }
...
//═══════════════════════════════════════════════════════════════════
// PAC — Market Structure processing
//═══════════════════════════════════════════════════════════════════
...
void AddOB(const datetime &time[], const double &open[],
   ...
   int sLen=bosBar-pivBar; if (sLen<1) return;
   double obTop=EMPTY_VALUE, obBtm=EMPTY_VALUE;
   int obIdx=-1; double obVol=0, totVol=0;
   for (int s=1; s<=sLen && (bosBar-s)>=0; s++) {
      int bi=bosBar-s; totVol+=(double)tvol[bi];
      if (bull && close[bi]<open[bi]) {
         if (obTop==EMPTY_VALUE||low[bi]<obBtm) { obTop=high[bi];obBtm=low[bi];obIdx=bi;obVol=(double)tvol[bi]; }
      }
      if (!bull && close[bi]>open[bi]) {
         if (obTop==EMPTY_VALUE||high[bi]>obTop) { obTop=high[bi];obBtm=low[bi];obIdx=bi;obVol=(double)tvol[bi]; }
      }
   }
   if (obIdx<0) return;
...
void ChkEq(double px, datetime t, bool isHigh) {
   ...
   int cnt=isHigh?iHiCnt:iLoCnt; if (cnt<2) return;
...
// IOFlow helpers
//═══════════════════════════════════════════════════════════════════
double IOVolSMA(const long &vol[], int idx, int period) {
   if (idx<period-1) return 1.0;
   double s=0; for (int i=0;i<period;i++) s+=(double)vol[idx-i]; return s/period;
}
double IOStr(double range, double brkDist, double relVol) {
   if (range<=0) return 0;
   double d=MathMin(brkDist/(range*5.0),1.0);
   double v=MathMin(relVol/2.0,1.0);
   return MathRound((d*0.6+v*0.4)*1000.0)/10.0;
}
bool IOIsOverlap(double top, double btm, const double &oT[], const double &oB[], int n) {
   for (int j=0;j<n;j++)
      if ((top<=oT[j]&&top>=oB[j])||(btm<=oT[j]&&btm>=oB[j])||(top>=oT[j]&&btm<=oB[j])) return true;
   return false;
}
...
void DrawAll(const datetime &time[], const double &high[], const double &low[], int rt) {
   ...
   for (int k=gOBCnt-1; k>=0; k--) {
      ...
      if (ShowMetrics) {
         string vt=DoubleToString(gOBs[k].volPct,1)+"K ("+DoubleToString(gOBs[k].volPct,1)+"%)";
         T(MkTxt(P"OM"+IntegerToString(k), rE, (gOBs[k].top+gOBs[k].btm)/2.0, vt, oc, 8));
      }
   }

   // ── IOFlow Order Blocks ──────────────────────────────────────────
   // Strongest OB (drawn across all history)
   double sT=EMPTY_VALUE, sB=EMPTY_VALUE; bool sBl=false; double maxS=-1.0; int chk=0;
   for (int k=g_ioN-1; k>=0; k--) {
      if (g_ioOB[k].mit) continue;
      if (g_ioOB[k].str>maxS) { maxS=g_ioOB[k].str; sT=g_ioOB[k].top; sB=g_ioOB[k].btm; sBl=g_ioOB[k].bull; }
      if (++chk>=BufferSize) break;
   }
   if (tIOStr && sT!=EMPTY_VALUE) {
      color sc=sBl?BullOBCol:BearOBCol;
      datetime t0ref=(rt>0)?time[0]:TimeCurrent();
      T(MkRect(P"IOStr", t0ref, sT, rE, sB, sc, 1, true));
      T(MkLine(P"IOMid", t0ref, (sT+sB)/2.0, rE, (sT+sB)/2.0, sc, 2, STYLE_DASH));
   }

   double occT[50], occB[50]; int occN=0, drawn=0;
   for (int k=g_ioN-1; k>=0 && drawn<MaxOFs; k--) {
      if (g_ioOB[k].mit) continue;
      if (!tIOBul &&  g_ioOB[k].bull) continue;
      if (!tIOBer && !g_ioOB[k].bull) continue;
      if (tIOOvlp && IOIsOverlap(g_ioOB[k].top, g_ioOB[k].btm, occT, occB, occN)) continue;
      if (occN<50) { occT[occN]=g_ioOB[k].top; occB[occN]=g_ioOB[k].btm; occN++; }
      drawn++;
      color oc=g_ioOB[k].bull ? C'0,180,140' : C'220,40,55';
      string suf=IntegerToString(k);
      T(MkRect(P"IO"+suf, g_ioOB[k].t0, g_ioOB[k].top, rE, g_ioOB[k].btm, oc, 1, true));
      if (tIOLbl) {
         T(MkTxt(P"IL"+suf, rE, (g_ioOB[k].top+g_ioOB[k].btm)/2.0,
                 DoubleToString(g_ioOB[k].str,1)+"%", oc, 8));
      }
   }
   ...
   if (tPD && sHiCnt>=1 && sLoCnt>=1) {
      double rHi=sHiPx[sHiCnt-1], rLo=sLoPx[sLoCnt-1];
      if (rHi>rLo) {
   ...
   if (tTLZ) {
      if (iLoCnt>=2) {
         double sl=iLoPx[iLoCnt-1]-iLoPx[iLoCnt-2];
         T(MkLine(P"TLZBI",  iLoPt[iLoCnt-2],iLoPx[iLoCnt-2],rE,iLoPx[iLoCnt-1]+sl*0.5,BullTLZCol,2,STYLE_SOLID));
      }
      if (iHiCnt>=2) {
         double sl=iHiPx[iHiCnt-1]-iHiPx[iHiCnt-2];
         T(MkLine(P"TLZBI2", iHiPt[iHiCnt-2],iHiPx[iHiCnt-2],rE,iHiPx[iHiCnt-1]+sl*0.5,BearTLZCol,2,STYLE_SOLID));
      }
      if (sLoCnt>=2) {
         double sl=sLoPx[sLoCnt-1]-sLoPx[sLoCnt-2];
         T(MkLine(P"TLZBS",  sLoPt[sLoCnt-2],sLoPx[sLoCnt-2],rE,sLoPx[sLoCnt-1]+sl*0.5,BullTLZCol,3,STYLE_SOLID));
      }
      if (sHiCnt>=2) {
         double sl=sHiPx[sHiCnt-1]-sHiPx[sHiCnt-2];
         T(MkLine(P"TLZBS2", sHiPt[sHiCnt-2],sHiPx[sHiCnt-2],rE,sHiPx[sHiCnt-1]+sl*0.5,BearTLZCol,3,STYLE_SOLID));
      }
   }
...
void CompactPACOBs() { int j=0; for(int i=0;i<gOBCnt;i++) if(!gOBs[i].mit)gOBs[j++]=gOBs[i]; gOBCnt=j; }
void CompactFVs()    { int j=0; for(int i=0;i<gFVCnt;i++) if(!gFVs[i].mit)gFVs[j++]=gFVs[i]; gFVCnt=j; }
void CompactIOOBs()  { int j=0; for(int i=0;i<g_ioN;i++)  if(!g_ioOB[i].mit)g_ioOB[j++]=g_ioOB[i]; g_ioN=j; }

double PivH(const double &h[],int idx,int len,int total) {
   if(idx-len<0||idx+len>=total) return 0;
   double v=h[idx]; for(int i=1;i<=len;i++) if(h[idx-i]>=v||h[idx+i]>=v) return 0; return v;}
double PivL(const double &l[],int idx,int len,int total) {
   if(idx-len<0||idx+len>=total) return 0;
   double v=l[idx]; for(int i=1;i<=len;i++) if(l[idx-i]<=v||l[idx+i]<=v) return 0; return v;}
...
void CreatePanel() {
   DelPanel();
   int w=240, x=PanelX, y=PanelY, h=482;
   PnlRect(P"Bg",x,y,w,h,C'13,17,23',C'30,45,61');

   PnlLbl(P"T1",x+10,y+9,"SMC Suite",         C'0,212,161', 10, true);
   PnlLbl(P"T2",x+10,y+26,"TraderLogJournal",  C'45,65,80',  7,  false);
   PnlSep(P"S0",x+8,y+40,w-16);

   PnlLbl(P"LMS",x+10,y+48,"MARKET STRUCTURE", C'74,96,117', 7, false);
   PnlToggle(BT_MSINT, x+8,y+60, w-16,24,"Market Structure Internal",tMSInt);
   PnlToggle(BT_MSSWG, x+8,y+87, w-16,24,"Market Structure Swing",   tMSSWg);
   PnlSep(P"S1",x+8,y+114,w-16);

   PnlLbl(P"LOB",x+10,y+122,"ORDER BLOCKS",    C'74,96,117', 7, false);
   PnlToggle(BT_OBINT, x+8,y+134,w-16,24,"OB Internal (PAC)",  tOBInt);
   PnlToggle(BT_OBSWG, x+8,y+161,w-16,24,"OB Swing (PAC)",     tOBSWg);
   PnlToggle(BT_IOBUL, x+8,y+188,w-16,24,"OF Bullish OBs",     tIOBul);
   PnlToggle(BT_IOBER, x+8,y+215,w-16,24,"OF Bearish OBs",     tIOBer);
   PnlToggle(BT_IOLBL, x+8,y+242,w-16,24,"OF Strength Labels", tIOLbl);
   PnlToggle(BT_IOSTR, x+8,y+269,w-16,24,"OF Strongest Zone",  tIOStr);
   PnlToggle(BT_IOOVLP,x+8,y+296,w-16,24,"Hide Overlapped",    tIOOvlp);
   PnlSep(P"S2",x+8,y+323,w-16);

   PnlLbl(P"LPT",x+10,y+331,"PATTERNS",        C'74,96,117', 7, false);
   PnlToggle(BT_EQHL,  x+8,y+343,w-16,24,"EQH / EQL",          tEQHL);
   PnlToggle(BT_FVG,   x+8,y+370,w-16,24,"Fair Value Gaps",     tFVG);
   PnlToggle(BT_PD,    x+8,y+397,w-16,24,"Premium / Discount",  tPD);
   PnlToggle(BT_TLZ,   x+8,y+424,w-16,24,"Trend Line Zones",    tTLZ);
   PnlSep(P"S3",x+8,y+451,w-16);
   PnlBtn(BT_RESET,x+8,y+457,w-16,18,"Reset & Recalculate",
...
void DelPanel() {
   string pn[]={P"Bg",P"T1",P"T2",P"S0",P"LMS",P"S1",P"LOB",P"S2",P"LPT",P"S3",
                BT_MSINT,BT_MSSWG,BT_OBINT,BT_OBSWG,
                BT_IOBUL,BT_IOBER,BT_IOLBL,BT_IOSTR,BT_IOOVLP,
...
void OnChartEvent(const int id,const long &lp,const double &dp,const string &sp) {
   ...
   else if (sp==BT_OBINT) {tOBInt =!tOBInt;  UpdTgl(BT_OBINT, tOBInt, "OB Internal (PAC)");}
   else if (sp==BT_OBSWG) {tOBSWg =!tOBSWg;  UpdTgl(BT_OBSWG, tOBSWg, "OB Swing (PAC)");}
   else if (sp==BT_IOBUL) {tIOBul =!tIOBul;  UpdTgl(BT_IOBUL, tIOBul, "OF Bullish OBs");}
   else if (sp==BT_IOBER) {tIOBer =!tIOBer;  UpdTgl(BT_IOBER, tIOBer, "OF Bearish OBs");}
   else if (sp==BT_IOLBL) {tIOLbl =!tIOLbl;  UpdTgl(BT_IOLBL, tIOLbl, "OF Strength Labels");}
   else if (sp==BT_IOSTR) {tIOStr =!tIOStr;  UpdTgl(BT_IOSTR, tIOStr, "OF Strongest Zone");}
   else if (sp==BT_IOOVLP){tIOOvlp=!tIOOvlp; UpdTgl(BT_IOOVLP,tIOOvlp,"Hide Overlapped");}
